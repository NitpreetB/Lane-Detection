...
while success:
    ...
    edges = cv2.Canny(transformed_frame, 50, 150)
    ...
    if lines is not None:
        for line in lines:
            ...
            pt1 = (int(x1), int(y1))
            pt2 = (int(x2), int(y2))
            # Check if line is within the region of interest
            if cv2.pointPolygonTest(roi_vertices, pt1, False) >= 0 and cv2.pointPolygonTest(roi_vertices, pt2, False) >= 0:
